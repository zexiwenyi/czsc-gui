"""缠论结构分析页面 V1.06 - 推送诊断增强 + 自动刷新TDX图表"""
from PyQt6.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout, QLabel, QTableWidget,
    QTableWidgetItem, QHeaderView, QSplitter, QFrame, QComboBox,
    QSlider, QLineEdit, QPushButton, QMessageBox, QSpinBox,
    QDoubleSpinBox
)
...
class StructureView(QWidget):
    ...
    def _init_ui(self):
        ...
        # === 波段对比面板 (V1.03) ===
        self.compare_panel = self._build_compare_panel()
        ...
        seg_title = QLabel("scipy波段段")
        seg_title.setStyleSheet(f"font-size: 14px; font-weight: bold; color: #ff9800;")
        seg_layout.addWidget(seg_title)

        self.seg_table = QTableWidget()
        self.seg_table.setColumnCount(6)
        self.seg_table.setHorizontalHeaderLabels(["序号", "方向", "起点", "终点", "起点价", "终点价"])
        self.seg_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        self.seg_table.setStyleSheet(self._table_style())
        seg_layout.addWidget(self.seg_table)
        table_splitter.addWidget(seg_frame)
    ...
    def _build_compare_panel(self):
        ...
        row.addWidget(self._lbl("波段对比:"))

        # distance 参数
        row.addWidget(self._lbl("distance:"))
        self.distance_spin = QSpinBox()
        self.distance_spin.setRange(3, 50)
        self.distance_spin.setValue(10)
        self.distance_spin.setFixedWidth(60)
        self.distance_spin.setStyleSheet(self._spin_style())
        row.addWidget(self.distance_spin)

        # prominence 参数
        row.addWidget(self._lbl("prominence:"))
        self.prominence_spin = QDoubleSpinBox()
        self.prominence_spin.setRange(0.0, 100.0)
        self.prominence_spin.setValue(0.0)
        self.prominence_spin.setSingleStep(0.5)
        self.prominence_spin.setDecimals(2)
        self.prominence_spin.setFixedWidth(70)
        self.prominence_spin.setStyleSheet(self._spin_style())
        row.addWidget(self.prominence_spin)

        row.addSpacing(10)

        # 分析对比按钮
        self.compare_btn = QPushButton("分析对比")
        self.compare_btn.setFixedSize(100, 30)
        self.compare_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self.compare_btn.setStyleSheet(self._btn_style("#ff9800"))
        ...
        self.compare_status.setStyleSheet(f"color: {COLORS['text_dim']}; font-size: 12px;")
        row.addWidget(self.compare_status)
    ...
    def _on_fetch_tdx(self):
        ...
        if not stock_code:
            self.tdx_status.setText("请输入有效股票代码")
            return
    ...
    def _on_push_tdx(self):
        ...
        from core.tdx_chart_push import push_to_tdx_with_period
        self.push_btn.setEnabled(False)
        ...
        import threading
        def _do():
            ok, msg = push_to_tdx_with_period(
    ...
    def _on_jump_tdx(self):
        ...
        if not stock_code:
            ...
            return
        self.stock_input.setText(stock_code)
        # 通达信 exec_to_tdx 要求功能串以 http://www.treeid/ 开头
        # 先切换到目标股票，再打开K线界面
        code_url = f"http://www.treeid/ZSCP?code={stock_code}"
        self.tdx_engine.exec_to_tdx(code_url)
        # 打开K线界面
        period = self.period_combo.currentText()
        period_map = {'1m': '0', '5m': '1', '15m': '2', '30m': '3', '1h': '4', '1d': '5'}
        p = period_map.get(period, '5')
        kline_url = f"http://www.treeid/KLINE?code={stock_code}&period={p}"
        self.tdx_engine.exec_to_tdx(kline_url)

    @staticmethod
    def _normalize_stock_code(code):
        """自动补全股票代码后缀：6xx→.SH, 0xx/3xx→.SZ, 4xx/8xx→.BJ"""
        ...
        if first in ('6', '9'):
            ...
            return f"{pure}.BJ"
        return f"{pure}.SH"  # 默认上海

    # ---- 波段对比分析 (V1.03) ----
    def _on_compare(self):
        """运行CZSC + scipy波段对比分析"""
        if not self.engine:
            self.compare_status.setText("请先加载数据")
            self.compare_status.setStyleSheet(f"color: {COLORS['red']}; font-size: 12px;")
            return

        self.compare_btn.setEnabled(False)
        self.compare_status.setText("分析中...")
        self.compare_status.setStyleSheet(f"color: {COLORS['yellow']}; font-size: 12px;")

        try:
            bar_data = self.engine.get_bar_data()
            distance = self.distance_spin.value()
            prominence_val = self.prominence_spin.value()
            prominence = prominence_val if prominence_val > 0 else None

            # 运行 scipy 波段检测
            detector = PeakDetector(bar_data, distance=distance, prominence=prominence)
            scipy_summary = detector.get_summary()

            # 获取 CZSC 统计
            czsc_summary = self.engine.get_bi_summary()

            # 更新统计摘要 - CZSC vs scipy 对比显示
            self._update_comparison_stats(czsc_summary, scipy_summary)

            # 绘制对比图
            self.kline_widget.plot_comparison(
                bar_data,
                czsc_bi_list=self.engine.bi_list,
                czsc_fx_list=self.engine.fx_list,
                peak_highs=detector.highs,
                peak_lows=detector.lows,
                peak_segments=detector.segments,
            )

            # 填充 scipy 波段段表
            self._fill_seg_table(detector.segments)

            self.compare_status.setText(
                f"CZSC {czsc_summary.get('笔总数', 0)}笔 | "
                f"scipy {scipy_summary['total_segments']}段 "
                f"(峰{scipy_summary['peaks']} 谷{scipy_summary['valleys']})"
            )
            self.compare_status.setStyleSheet(f"color: {COLORS['green']}; font-size: 12px;")

        except Exception as e:
            self.compare_status.setText(f"分析失败: {e}")
            self.compare_status.setStyleSheet(f"color: {COLORS['red']}; font-size: 12px;")
        finally:
            self.compare_btn.setEnabled(True)

    def _update_comparison_stats(self, czsc_summary, scipy_summary):
        """更新对比统计摘要"""
        ...
        # CZSC 统计
        czsc_items = [
            ("CZSC K线", str(self.engine.n_bars)),
            ("CZSC 分型", str(len(self.engine.fx_list))),
            ("CZSC 笔", str(czsc_summary.get("笔总数", 0))),
            ("CZSC 均幅", str(czsc_summary.get("平均幅度", 0))),
        ]
        # scipy 统计
        scipy_items = [
            ("scipy 段数", str(scipy_summary['total_segments'])),
            ("scipy 峰", str(scipy_summary['peaks'])),
            ("scipy 谷", str(scipy_summary['valleys'])),
            ("scipy 均幅", str(scipy_summary['avg_amplitude'])),
        ]

        # CZSC section header
        czsc_header = QLabel(f"<b style='color:{COLORS['accent']}'>[CZSC缠论]</b>")
        czsc_header.setStyleSheet("font-size: 13px;")
        self.stats_layout.addWidget(czsc_header)
        for label, value in czsc_items:
        ...
        # 分隔
        sep = QLabel("  |  ")
        sep.setStyleSheet(f"color: {COLORS['sidebar_hover']}; font-size: 13px;")
        self.stats_layout.addWidget(sep)

        # scipy section header
        scipy_header = QLabel(f"<b style='color:#ff9800'>[scipy波段]</b>")
        scipy_header.setStyleSheet("font-size: 13px;")
        self.stats_layout.addWidget(scipy_header)
        for label, value in scipy_items:
            w = QLabel(f"<b style='color:#ff9800'>{value}</b>  <span style='color:{COLORS['text_dim']}'>{label}</span>")
            w.setStyleSheet("font-size: 13px;")
            self.stats_layout.addWidget(w)

        self.stats_layout.addStretch()

    def _fill_seg_table(self, segments):
        """填充scipy波段段表格"""
        self.seg_table.setRowCount(len(segments))
        for i, seg in enumerate(segments):
            self.seg_table.setItem(i, 0, QTableWidgetItem(str(i)))
            dir_text = "上涨" if seg['direction'] == 'up' else "下跌"
            dir_item = QTableWidgetItem(dir_text)
            if seg['direction'] == 'up':
                dir_item.setForeground(pg.mkColor("#ff9800"))
            else:
                dir_item.setForeground(pg.mkColor("#ce93d8"))
            self.seg_table.setItem(i, 1, dir_item)
            start_dt = str(seg['start_dt'])[:16] if hasattr(seg['start_dt'], 'strftime') else str(seg['start_dt'])[:16]
            end_dt = str(seg['end_dt'])[:16] if hasattr(seg['end_dt'], 'strftime') else str(seg['end_dt'])[:16]
            self.seg_table.setItem(i, 2, QTableWidgetItem(start_dt))
            self.seg_table.setItem(i, 3, QTableWidgetItem(end_dt))
            self.seg_table.setItem(i, 4, QTableWidgetItem(str(round(seg['start_price'], 2))))
            self.seg_table.setItem(i, 5, QTableWidgetItem(str(round(seg['end_price'], 2))))
