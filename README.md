...
| V1.03 | `czsc_gui_v103/` | 对比版：新增 scipy.signal 波段检测对比分析 |
| V1.04 | `czsc_gui_v104/` | 修复版：修复通达信跳转K线URL格式，修正公式变量名冲突 |
...
│   └── widgets/
│       └── kline_widget.py    # pyqtgraph K线图组件
├── czsc_gui_v103/             # V1.03 - scipy波段对比版
│   ├── main.py                # 主入口（同 V1.02）
│   ├── config.py              # 全局配置
│   ├── core/
│   │   ├── czsc_engine.py     # CZSC 分析引擎
│   │   ├── data_loader.py     # CSV 数据加载
│   │   ├── tdx_engine.py      # 通达信数据引擎
│   │   ├── tdx_chart_push.py  # CZSC→TDX 推送桥接
│   │   └── peak_detector.py   # [新] scipy 波段检测器
│   ├── views/
│   │   ├── overview.py        # 平台总览
│   │   └── structure.py       # 缠论结构分析（新增对比面板）
│   └── widgets/
│       └── kline_widget.py    # K线图（新增对比叠加）
├── czsc_gui_v104/             # V1.04 - 修复版
│   ├── (基于 V1.03，修复跳转URL和公式变量名)
│   └── ...
```
